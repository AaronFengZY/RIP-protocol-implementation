...
class DVRouter(DVRouterBase):
    ...
    def send_routes(self, force=False, single_port=None):
        ...
        for port in self.ports.get_all_ports():
            for host, entry in self.table.items():
                if not self.POISON_REVERSE or entry.port!=port:
    ...
    def expire_routes(self):
        ...
        for host,entry in self.table.items():
            if entry.expire_time > api.current_time():
                ...
                renew_table[host] =TableEntry(dst=entry.dst, port=entry.port, latency=INFINITY,
    ...
    def handle_route_advertisement(self, route_dst, route_latency, port):
        ...
        if route_latency == INFINITY: #stage 8
            if route_dst in self.table and self.table[route_dst].port == port:
                ...
                self.table[route_dst] = TableEntry(dst = route_dst, port=port, latency=INFINITY,expire_time=expire_time_now)
        else:
            new_table = TableEntry(dst=route_dst, port=port,latency=route_latency+self.ports.get_latency(port), 
                    expire_time=api.current_time()+self.ROUTE_TTL)
            if not self.table.get(route_dst): 
                self.table[route_dst] = new_table
            else: #update if the latency is lower or that is the port
                entry = self.table[route_dst]
